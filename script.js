...
// Capture the default cursor image
const defaultCursor = 'default.png';
...
let fruits = [];
let points = 0;
const fruitsList = ["bluefish", "yellowfish", "orangefish"];
//Events object
let events = {
  mouse: {
    down: "mousedown",
...
function Fruit(image, x, y, width) {
  ...
  this.update = () => {
    this.x += this.speed;
    if (!this.complete && this.x + this.width > canvas.height) {
      this.complete = true;
    }
  };
  //Draw fruit
...
// Function to change cursor image
...
function createRandomFruit() {
  ...
  // Create multiple fruits at the same time
  const numNewFruits = generateRandomNumber(1, 3); // Change the range for the number of fruits
  for (let i = 0; i < numNewFruits; i++) {
    if (fruits.length < FRUIT_COUNT) {
      ...
      let fruit = new Fruit(randomImage, 0, randomX, fruitWidth);
      fruits.push(fruit);
    }
  }
...
// Restore default cursor when mouse is released
document.addEventListener('mouseup', function () {
    changeCursorImage(defaultCursor);
});
